#include "RequestAddBlockedIPAddressHandler.h"
...
void NetworkDataHandler::RequestAddBlockedIPAddress::handle(IConnectionPtr con, const unsigned char *data, size_t len)
{
    ...
    USUAL_PARSE_PROTOBUF_DATA_MACRO(Global::Protocol::Server::Management::RequestAddBlockedIPAddress, request, data, len, usrcon->getSecondCryptology())

    BlackList::share()->addIPAddress(request.ip_address(), request.duration());

    ApplicationServer::share()->kickout(request.ip_address());
    ...
    GLOBAL_PROTO_WRAPPER_DESC_WITH_ENCRYPT(GLOBAL_PROTOCOL_NETWORK_TAG_RESPONSE_ADD_BLOCKED_IP_ADDRESS, response, responseData, usrcon->getSecondCryptology())
