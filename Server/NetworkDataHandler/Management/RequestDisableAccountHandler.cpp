#include "RequestDisableAccountHandler.h"
...
void NetworkDataHandler::RequestDisableAccount::handle(IConnectionPtr con, const unsigned char *data, size_t len)
{
    ...
    USUAL_PARSE_PROTOBUF_DATA_MACRO(Global::Protocol::Server::Management::RequestDisableAccount, request, data, len, usrcon->getSecondCryptology())

    Global::Protocol::Server::Management::ResponseDisableAccount response;

    int err = AccountManager::share()->enableAccount(request.account());
    ...
    GLOBAL_PROTO_WRAPPER_DESC_WITH_ENCRYPT(GLOBAL_PROTOCOL_NETWORK_TAG_RESPONSE_DISABLE_ACCOUNT, response, responseData, usrcon->getSecondCryptology())
