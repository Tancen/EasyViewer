#include "RequestRemoveBlockedAccountHandler.h"
...
void NetworkDataHandler::RequestRemoveBlockedAccount::handle(IConnectionPtr con, const unsigned char *data, size_t len)
{
    ...
    USUAL_PARSE_PROTOBUF_DATA_MACRO(Global::Protocol::Server::Management::RequestRemoveBlockedAccount, request, data, len, usrcon->getSecondCryptology())

    BlackList::share()->removeAccount(request.user_id());
    ...
    GLOBAL_PROTO_WRAPPER_DESC_WITH_ENCRYPT(GLOBAL_PROTOCOL_NETWORK_TAG_RESPONSE_REMOVE_BLOCKED_ACCOUNT, response, responseData, usrcon->getSecondCryptology())
